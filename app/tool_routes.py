...
def sbahn():
    ...
    with open(drive_dir, encoding="utf8") as html_file:
        ...
        # Find classes with text as classes change from device to device
        class_username = soup.find(string="gruettecloud").find_parent("span").get("class")
        class_username = " ".join(class_username)
        
        class_comments = soup.find(string="Die Rolltreppe funktioniert besser als die Witze in den Memes").find_parent("div").get("class")
        class_comments = " ".join(class_comments)

        usernames = soup.find_all("span", class_=class_username)
        comments = soup.find_all("div", class_=class_comments)
