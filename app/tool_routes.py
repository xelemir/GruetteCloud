...
def zuffenhausen():
    if request.method == "GET":
        ...
        Thread(target=MailHelper.MailHelper().send_email, args=(email_jan, "jan", f"{visitor1} wants to visit on {date.strftime('%d.%m.%Y %H:%M')}", f"{visitor1} wants to visit you on {date.strftime('%d.%m.%Y %H:%M')}. They are coming for the following reasons: {reasons}. {accompaniedBy}.<br><br><a href='https://www.gruettecloud.com/zuffenhausen/modify?type=approve&id={application_id}' style='text-decoration: none; color: #0A84FF;'>Approve</a> | <a href='https://www.gruettecloud.com/zuffenhausen/modify?type=deny&id={application_id}' style='text-decoration: none; color: #0A84FF;'>Deny</a><br><br>", "None", "https://www.gruettecloud.com/static/gruettecloud_logo.png", True)).start()
...
def zuffenhausen_modify():
    ...
    if request.args.get("type") == "approve":
        ...
        date = datetime.datetime.strptime(str(visit["date"]), "%Y-%m-%d %H:%M")
        date = date.strftime("%d.%m.%Y %H:%M")
